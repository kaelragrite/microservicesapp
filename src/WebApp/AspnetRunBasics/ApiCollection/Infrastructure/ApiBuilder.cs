...
namespace AspnetRunBasics.ApiCollection.Infrastructure
{
    public class ApiBuilder
    {
        ...
        {
            if (string.IsNullOrEmpty(_builder.Path) || _builder.Path == "/") _builder.Path = path;
            else
            {
        ...
        public bool HasSubdomain() => _builder.Uri.HostNameType == UriHostNameType.Dns && _builder.Uri.Host.Split('.').Length > 2;
        ...
        public ApiBuilder AddQueryString(string name, string value)
        {
            ...
            _builder.Query = qsNv.ToString() ?? string.Empty;
            return this;
        }
        
        public ApiBuilder QueryString(string queryString)
        {
            if (!string.IsNullOrEmpty(queryString)) _builder.Query = queryString;
            return this;
        }
        ...
        public string GetLeftPart() => _builder.Uri.GetLeftPart(UriPartial.Path);
    }
}